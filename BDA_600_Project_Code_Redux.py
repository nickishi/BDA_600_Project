...
def api_call_list(ticker_list):
    ...
    for company in ticker_list:
        ...
        df2 = comp_data.loc["2020-12-30" : "2022-12-31"]

        print(df2)
        df2 = df2["5. adjusted close"]
        df2.rename(columns={"5. adjusted close": company})
        print(df2)
        df2.to_csv(company + "_Monthly_Adjusted_2020_2022.csv")

        # df2 = df2.to_frame()
        # print(df2)
        # df2.rename(columns = {"5. adjusted close":company})
        #
        # if os.path.isfile("All_Companies_Adj_Close_2020_2022.csv"):
        #     df2.to_csv("All_Companies_Adj_Close_2020_2022.csv", index=False, mode="a")
        # else:
        #     df2.to_csv("All_Companies_Adj_Close_2020_2022.csv")
...
def main():
    ...
    while exit == "yes":
        ...
        if ticker.upper() == "LIST":
            ...
            print("1")
            list_of_companies = ["AAPL", "MSFT", "NVDA", "UNH", "JNJ", "MRK", "RTX", "HON", "UPS", "BRK.B", "JPM", "BAC"]
